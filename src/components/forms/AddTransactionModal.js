...
const EXPENSE_CATEGORIES = ["Food", "Travel", "Bills", "Savings", "Lifestyle"];
const INCOME_CATEGORIES = ["Salary", "Freelance", "Mentoring", "Gift"];

export default function AddTransactionModal({ isOpen, onClose, onSuccess }) {
  const [loading, setLoading] = useState(false);
  const [type, setType] = useState("EXPENSE"); // 'INCOME' or 'EXPENSE'
  const [isCustomCategory, setIsCustomCategory] = useState(false); // To toggle "Other" input
  ...
  // Reset category if user switches between Income/Expense
  useEffect(() => {
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              {!isCustomCategory ? (
                <select
                  required
                  className="w-full mt-1 px-3 py-2 border rounded-lg focus:ring-2 focus:ring-slate-900 outline-none text-sm bg-white"
                  value={formData.category}
                  onChange={(e) => {
                    const val = e.target.value;
                    if (val === "Other") {
                      setIsCustomCategory(true);
                      setFormData({ ...formData, category: "" }); // Clear so user can type
                    } else {
